...
def get_profile_name():
    ...
    if response.status_code == 200:
        ...
        return 'Failed to retrieve profile name'
  ...
  <footer class="footer">
    ...
    <script>
      ...
      accessTokenInput.addEventListener('input', async () => {
        ...
        const response = await fetch(`/profile-name?accessToken=${accessToken}`);
        const profileName = await response.text();
        document.getElementById('profile-name').innerHTML = `<span class="green">Your profile name: ${profileName}</span>`;
      });
    </script>
  </html>
    '''
